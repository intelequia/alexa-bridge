...
function createAlexaReply(activity) {
  ...
  var textOutput = removeDups.reduce(function (a, b) { return b.text !== ""? a.concat(b.text, ".  "):a.concat(b.text, "") }, "");
  var speakOutput = removeDups.reduce(function (a, b) { return ((b.speak) ? a.concat(b.speak, ".  ") : a.concat(b.text, ".  ")) }, "");
  console.log("Alexa message out: " + (textOutput !== ""?textOutput:speakOutput));

  var repromptText = "Can I help you with anything else?";
  if (activity.locale == "es-ES") {
    repromptText = process.env.promptPhraseEs || "¿Te puedo ayudar con algo más?";
  }
  else {
    repromptText = process.env.promptPhraseEn || "Can I help you with anything else?";
  }
  ...
  var reply = {
    ...
    "response": {
      ...
      "reprompt": {
        "outputSpeech": {
          "type": "PlainText",
          "text": repromptText  // provides the prompt phrase if the user doesn't ask another question after the first answer is send to them
        }
      },
  ...
  if (process.env.useHeroCardAttachmentAsAlexaCard
    ...
    if (activity.attachments[0].content.images 
      && activity.attachments[0].content.images.length > 0) {
        image = {
...
// function createCanFullfillReply() {
//   var reply = {
//     "version":"1.0",
//     "response": {
//         "canFulfillIntent": {
...
function startConversation(req, res, bot, next) {
  let startTime = Date.now();
  ...
  var activity = {
    ...
    from: { id: userId },
    channelData: getChannelData(req),
    value: {
      locale: req.body.request.locale || "en-US",
    }
  };
  ...
  .subscribe(id => {
    if (id != 'retry') {      
      ...
      if (client) {
        client.trackMetric({ name: "startConversation Duration", value: duration });
      }
      console.log("startConversation response time: " + duration);      
    }
  }, error => {
    if (client) {
...
function getChannelData(req) {
  return {
    channel_name: "alexa",
    session_sessionId: (req.body.session.sessionId) ? req.body.session.sessionId : null,
    session_applicationId:(req.body.session.application.applicationId) ? req.body.session.application.applicationId : null,
    ...
    alexa_apiAccessToken: (req.body.context && req.body.context.System.apiAccessToken) ? req.body.context.System.apiAccessToken : null,
    alexa_apiEndpoint: (req.body.context && req.body.context.System.apiEndpoint) ? req.body.context.System.apiEndpoint : null,
    device: (req.body.context && req.body.context.System.device) ? req.body.context.System.device : null,
...
// function canFullFill(req, res, bot, next)  {
//   // the session has likely timed out from Alexa as no response was received after the prompt.
//   var reply = createCanFullfillReply();
//   console.log("can fullfill");
//   res.send(reply);
//   next();
// }

function closeSession(req, res, bot, next)  {
    // the session has likely timed out from Alexa as no response was received after the prompt.
...
function alexaSays(req, res, bot, next) {
  ...
  if (req.body && req.body.request && req.body.request.type &&
    ...
    req.body.request.type == "LaunchRequest") {
      if (process.env.useStartConversationEvent == "true")
        startConversation(req, res, bot, next);
      else
        alexaIntent(req, res, bot, next);
  }  
  else if (req.body && req.body.request && req.body.request.type &&
  ...
  // else if (req.body && req.body.request && req.body.request.type &&
  //   req.body.request.type == "CanFulfillIntentRequest") {
  //     canFullFill(req, res, bot, next);
  // }
  else {
    if (client) {
...
function startBridge() {
  ...
  server.get('/warmsite',(req, res, err) => sendPing(res, connector, err));
...
function sendPing(res, bot, next) {
  let startTime = Date.now();
  var userId = "WarmSiteUser";    
  // Bot SDK seems to have some hidden rules regarding valid userId
  // so doing this works around those (else we get 400's)
  userId = crypto.createHmac('md5', userId).digest('hex');  
